...
Partial Class Team_Fixtures
    ...
    Function get_league_from_team(ByVal inLeague As String) As String
        ...
        strSQL = "SELECT league FROM ladies_skit.vw_teams WHERE long_name = '" & objGlobals.TeamSelected & "'"
    ...
    Sub load_title()
        ...
        strSQL = "SELECT * FROM ladies_skit.vw_tables WHERE league='" & objGlobals.LeagueSelected & "' AND team='" & objGlobals.TeamSelected & "'"
        myDataReader = objGlobals.SQLSelect(strSQL)
        While myDataReader.Read()
        ...
        hlTeamStats.NavigateUrl = "~/Ladies_Skit/Stats.aspx?League=" & objGlobals.LeagueSelected & "&Team=" & objGlobals.TeamSelected
    ...
    Sub get_week_pts_pld()
        ...
        strSQL = "SELECT Week,pts,pos FROM ladies_skit.vw_tables_week"
    ...
    Sub load_fixtures1(inGrid As GridView)
        ...
        strSQL = "SELECT venue FROM ladies_skit.vw_teams WHERE league = '" & objGlobals.LeagueSelected & "' AND long_name = '" & objGlobals.TeamSelected & "'"
        myDataReader = objGlobals.SQLSelect(strSQL)
        While myDataReader.Read()
        ...
        strSQL = "EXEC ladies_skit.sp_get_team_fixtures '" & objGlobals.get_current_season & "','" & objGlobals.LeagueSelected & "','" & objGlobals.TeamSelected & "'"
        myDataReader = objGlobals.SQLSelect(strSQL)
    ...
    Sub load_players()
        ...
        strSQL = "SELECT player FROM ladies_skit.vw_players WHERE league = '" & objGlobals.LeagueSelected & "' AND team = '" & objGlobals.TeamSelected & "' AND CHARINDEX( 'A N OTHER',Player) = 0  ORDER BY LEFT(Player,3)"
        myDataReader = objGlobals.SQLSelect(strSQL)
            ...
            If dt.Rows(gRow)(0) <> "" Then
                ...
                Else
                    ...
                    Select Case True
                        Case SinglesEntry
                            Dim hLink As New HyperLink
                            hLink.NavigateUrl = "~/Ladies_Skit/Stats.aspx?League=" & objGlobals.LeagueSelected & "&Team=" & lblTeam.Text & "&Player=" & dt.Rows(gRow)(0)
                            hLink.Text = e.Row.Cells(0).Text
                        ...
                        Select Case Status
                            Case 2
                                If objGlobals.AdminLogin Then
                                    hLink3.NavigateUrl = "~/Ladies_Skit/Admin/Add Edit Result 2.aspx?ID=" & Val(dt.Rows(gRow)(12)).ToString & "&Week=" & Val(dt.Rows(gRow)(0)).ToString & "&League=" & objGlobals.LeagueSelected & "&Team=" & objGlobals.TeamSelected
                                Else
                                    If InStr(dt.Rows(gRow)(2), "ROSSER", CompareMethod.Text) = 0 Then
                                        hLink3.NavigateUrl = "~/Ladies_Skit/Team Fixtures.aspx?League=" & objGlobals.LeagueSelected & "&Team=" & objGlobals.TeamSelected & "&CompID=" & Val(dt.Rows(gRow)(12)).ToString & "&FixtureType=League"
                                    Else
                                        hLink3.NavigateUrl = "~/Ladies_Skit/Team Fixtures.aspx?League=" & objGlobals.LeagueSelected & "&Team=" & objGlobals.TeamSelected & "&CompID=" & Val(dt.Rows(gRow)(12)).ToString & "&FixtureType=ARosser"
                                    End If
                                End If
                            ...
                            Case 0
                                hLink3.NavigateUrl = "~/Ladies_Skit/Admin/Fixture Result.aspx?ID=" & Val(dt.Rows(gRow)(12)).ToString & "&Week=" & Val(dt.Rows(gRow)(0)).ToString & "&League=" & objGlobals.LeagueSelected & "&Team=" & objGlobals.TeamSelected
    ...
    Protected Sub btnCard_Click(sender As Object, e As EventArgs) Handles btnCard.Click
        ...
        Dim filename As String = "LADIES SKITTLES RESULT CARD A4.pdf"
        Dim PDFfile As String = FilePath + filename
