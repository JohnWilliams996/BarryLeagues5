...
Partial Class Club_Fixtures
    ...
    Sub load_teams()
        ...
        TeamCount = 0
        strSQL = "SELECT league,long_name,home_night FROM mens_skit.vw_teams WHERE venue = '" & ClubSelected & "' ORDER BY league,long_name"
    ...
    Sub grid_load_dates()
        ...
        strSQL = "SELECT week_number,week_commences FROM mens_skit.vw_weeks ORDER BY week_commences"
        myDataReader = objGlobals.SQLSelect(strSQL)
        While myDataReader.Read()
            For intI = 0 To 4
                dr = dt.NewRow
    ...
    Sub grid_load_fixtures()
        ...
        For intI = 1 To TeamCount
            gCol = gCol + 1
            strSQL = "SELECT * FROM mens_skit.vw_fixtures WHERE (home_team_name = '" & TeamName(intI) & "' OR away_team_name = '" & TeamName(intI) & "' OR ISNUMERIC(home_team) = 0) ORDER BY week_number"
            myDataReader = objGlobals.SQLSelect(strSQL)
            While myDataReader.Read()
