...
Partial Class Admin_High_Scores
    ...
    Sub load_home_away()
        ...
        Select Case Left(ddLeague.Text, 4)
            Case "CRIB"
                ddHomeAway.Items.Add("CRIB")
            Case "SKIT"
                ddHomeAway.Items.Add("SKIT")
            Case "SNOO"
                ddHomeAway.Items.Add("Home")
                ddHomeAway.Items.Add("Away")
        End Select
    End Sub
    ...
    Sub load_teams()
        ...
        Dim myDataReader As oledbdatareader
        ddTeam.Items.Clear()
        strSQL = "SELECT DISTINCT(long_name) FROM clubs.vw_teams WHERE league = '" & ddLeague.Text & "' AND long_name <> 'BYE'"
    ...
    Sub load_players()
        ...
        strSQL = "SELECT player FROM clubs.vw_players WHERE league = '" & ddLeague.Text & "' AND team = '" & ddTeam.Text & "'"
    ...
    Protected Sub btnAddHS_Click(sender As Object, e As System.EventArgs) Handles btnAddHS.Click
        ...
        strSQL = "INSERT INTO clubs.high_scores VALUES ("
        ...
        Else
            strSQL = strSQL + "0,"
        End If
        strSQL = strSQL + "'" + UKDate + UKTime + "')"
        myDataReader = objGlobals.SQLSelect(strSQL)
        objGlobals.close_connection()
        Response.Redirect("~/Clubs/Admin/High Scores.aspx")
    End Sub
